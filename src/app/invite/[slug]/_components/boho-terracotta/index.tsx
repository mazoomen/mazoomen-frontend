...
interface InvitationClientPageBohoTerracottaProps {
...
export default function InvitationClientPageBohoTerracotta({
  invitation: initialInvitation,
  slug,
  isDeactivatedInitial = false,
  viewingLangProp,
  setViewingLangProp,
}: InvitationClientPageBohoTerracottaProps) {
  const [invitation, setInvitation] = useState<InvitationData>(initialInvitation);
  const [isOpen, setIsOpen] = useState(false);
  const [musicPlaying, setMusicPlaying] = useState(false);
  const [snowflakes, setSnowflakes] = useState<{ size: number; left: number; delay: string; duration: number; border: string; bg: string }[]>([]);
  ...
  useEffect(() => {
    // Generate organic leafy particles with borders and colors of terracotta/warm beige
    const flakes = Array.from({ length: 20 }).map((_, idx) => {
      ...
      return {
        ...
        border: isRust ? '50% 0 50% 0' : '50%',
        bg: isRust ? 'rgba(138, 78, 53, 0.4)' : 'rgba(210, 180, 140, 0.35)',
      };
    });
  ...
  return (
    <main className="min-h-screen bg-[#FAF5EE] relative flex flex-col justify-center" dir={isEn ? "ltr" : "rtl"}>
      ...
      {invitation.languageMode === "both" && (
        <button
          ...
          style={{
            ...
            borderColor: 'rgba(138, 78, 53, 0.35)',
            color: '#8A4E35',
            boxShadow: 'rgba(138, 78, 53, 0.15) 0px 4px 20px',
          }}
        >
        ...
        {snowflakes.map((flake, idx) => (
          <div
            ...
            style={{
              ...
              backgroundColor: flake.bg,
              borderRadius: flake.border,
      ...
      <div className="relative w-full max-w-md md:max-w-xl lg:max-w-2xl mx-auto overflow-hidden bg-white shadow-2xl rounded-none md:rounded-[32px] md:my-8" dir={isEn ? "ltr" : "rtl"} style={{ color: '#8A4E35' }}>
        {/* Hero Banner Section */}
        <InvitationHero
        ...
        <div className="fixed inset-0 bg-[#2D3142]/40 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <Phone className="w-4 h-4 text-[#8A4E35]" />
