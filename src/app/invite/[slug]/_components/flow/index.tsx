                ...
                <form onSubmit={handleRsvpSubmit} className={`space-y-4 ${isEn ? "text-left" : "text-right"}`}>
                  ...
                  {attendance === 'YES' && (
                    <div className="animate-fade-in">
                      <label htmlFor="companions-count" className="block text-xs font-semibold text-[#701020] mb-1">{isEn ? "Number of companions" : "عدد المرافقين"}</label>
                      <select
                        id="companions-count"
                        value={companionsCount}
                        onChange={(e) => setCompanionsCount(Number(e.target.value))}
                        className="w-full px-4 py-2.5 rounded-xl border border-[#701020]/15 bg-white/70 text-[#701020] text-xs focus:outline-none focus:ring-1 focus:ring-[#B07070]"
                      >
