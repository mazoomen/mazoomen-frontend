      ...
      <div className="relative w-full max-w-md md:max-w-xl lg:max-w-2xl mx-auto overflow-hidden bg-white shadow-2xl rounded-none md:rounded-[32px] md:my-8 animate-on-scroll" dir={isEn ? "ltr" : "rtl"} style={{ color: '#1B3222' }}>
        ...
        <section className="relative min-h-[700px] flex flex-col items-center justify-start text-center pt-24">
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
            <video
              ref={videoRef}
              src={`${S3_BASE_URL}/templates/videos/Untitled_design_6af89e10.mp4`}
              autoPlay
              loop
              playsInline
              muted
                ...
                <form onSubmit={handleRsvpSubmit} className={`space-y-4 ${isEn ? "text-left" : "text-right"}`}>
                  ...
                  {invitation.allowCompanions !== false && attendance === 'YES' && (
                    <div className="animate-fade-in space-y-2">
                      <label className="block text-xs font-semibold text-[#1B3222] mb-1">{isEn ? "Number of companions" : "عدد المرافقين"}</label>
                      <div className="flex items-center gap-3">
                        <button
                          type="button"
                          onClick={() => setCompanionsCount(prev => Math.max(1, prev - 1))}
                          className="w-10 h-10 rounded-lg border border-[#1B3222]/15 bg-white flex items-center justify-center text-sm font-bold text-[#1B3222] cursor-pointer hover:bg-white/80"
                        >
                          -
                        </button>
                        <div className="flex-grow text-center py-2.5 rounded-lg border border-[#1B3222]/10 bg-white/60 text-xs font-bold text-[#1B3222]">
                          {companionsCount}
                        </div>
                        <button
                          type="button"
